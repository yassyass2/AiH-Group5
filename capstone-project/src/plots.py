...
MLflow run (src/model.py logs every epoch there), then writes the figures to
``artifacts/figures/``:
...
DEFAULT_REPORT = PROJECT_DIR / "artifacts" / "test_report.json"
...
def load_history_from_mlflow(
    ...
    ``src/model.py`` logs every epoch of every history key as a step metric, so

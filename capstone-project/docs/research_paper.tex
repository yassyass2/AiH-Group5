...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    stage/.style={draw, rounded corners, align=center, minimum width=34mm, minimum height=10mm, fill=blue!7},
    branch/.style={draw, rounded corners, align=center, minimum width=34mm, minimum height=10mm, fill=green!8},
    eval/.style={draw, rounded corners, align=center, minimum width=36mm, minimum height=10mm, fill=orange!10},
    warn/.style={draw, rounded corners, align=center, minimum width=40mm, minimum height=10mm, fill=red!8},
    arrow/.style={-Latex, thick}
  ]
    \node[stage] (data) {APTOS 2019\\fundus images\\5 DR grades};
    \node[stage, right=of data] (prep) {Preprocessing\\crop, resize, green\\CLAHE, median blur};
    \node[stage, right=of prep] (splits) {Fixed arrays\\train / validation / test\\\((224,224,1)\)};

    \node[branch, below left=12mm and -3mm of splits] (b0) {B0 baseline\\class weights};
    \node[branch, right=of b0] (b1b3) {Backbone ablation\\B1 and B3\\class weights};
    \node[branch, right=of b1b3] (smote) {SMOTE ablation\\training split only};
    \node[branch, right=of smote] (ordinal) {Ordinal regression\\severity score\\QWK thresholds};

    \node[eval, below=14mm of b1b3] (metrics) {Evaluation\\QWK, accuracy\\per-class Se/Sp\\macro AUC};
    \node[eval, right=of metrics] (xai) {Explainability audit\\Grad-CAM\\Grad-CAM++\\Score-CAM};
    \node[warn, below=13mm of metrics, xshift=28mm] (selected) {Selected model\\EfficientNetB3\\QWK \(0.8908\)\\severe-recall caveat};

    \draw[arrow] (data) -- (prep);
    \draw[arrow] (prep) -- (splits);
    \draw[arrow] (splits) |- (b0);
    \draw[arrow] (splits) |- (b1b3);
    \draw[arrow] (splits) |- (smote);
    \draw[arrow] (splits) |- (ordinal);
    \draw[arrow] (b0) -- (metrics);
    \draw[arrow] (b1b3) -- (metrics);
    \draw[arrow] (smote) |- (metrics);
    \draw[arrow] (ordinal) |- (metrics);
    \draw[arrow] (metrics) -- (selected);
    \draw[arrow] (xai) -- (selected);
    \draw[arrow] (b1b3) |- (xai);

    \node[draw, dashed, rounded corners, fit=(b0)(b1b3)(smote)(ordinal), inner sep=5mm, label={[font=\small]above:Experimental branches}] {};
  \end{tikzpicture}
  }

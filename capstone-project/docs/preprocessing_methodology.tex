...
the input resolution of the target network, (iii)~green-channel extraction
followed by Contrast Limited Adaptive Histogram Equalisation (CLAHE) for
contrast enhancement, (iv)~median filtering for noise reduction,
(v)~normalisation of pixel intensities to the unit interval, and
(vi)~synthetic minority over-sampling (SMOTE) applied exclusively to the
training partition. The resulting tensors are stored in a
...
\begin{enumerate}
  ...
  \item \textbf{Noise reduction.} Median filtering to suppress sensor noise
        while preserving retinal structures (Section~\ref{sec:denoise}).
...
Colour fundus photographs carry sensor and compression noise that the local
contrast enhancement of Section~\ref{sec:clahe} can accentuate. We therefore
apply a median filter with a $3\times3$ kernel to the enhanced image. Median
filtering removes isolated speckle noise while, unlike a Gaussian blur,
preserving the edges of vessels and lesions, which are the diagnostically
relevant structures. The small kernel keeps fine detail intact. The filter is
applied while the image is still in 8-bit form, as it precedes the
